...
fn create_branch(repo: &gix::Repository, branch: &str, head_id: gix::ObjectId) {
    git::branch(repo, branch, &head_id);
}

fn create_tag(repo: &gix::Repository, tag: &str, _head_id: gix::ObjectId) {
    git::tag(repo, tag);
}

fn v3_and_git_url_encoded(ref_kind: &str, f: impl FnOnce(&gix::Repository, &str, gix::ObjectId)) {
    let (git_project, repo) = git::new_repo("dep1", |project| {
        project
            .file("Cargo.toml", &basic_lib_manifest("dep1"))
            .file("src/lib.rs", "")
...
fn v4_and_git_url_encoded(ref_kind: &str, f: impl FnOnce(&gix::Repository, &str, gix::ObjectId)) {
    let (git_project, repo) = git::new_repo("dep1", |project| {
        project
            .file("Cargo.toml", &basic_lib_manifest("dep1"))
            .file("src/lib.rs", "")
